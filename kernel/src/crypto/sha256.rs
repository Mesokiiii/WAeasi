//! SHA-256 — standalone, no `alloc` on the hot path.
//!
//! Reference: FIPS 180-4.  This implementation is the standard
//! single-block compression with the canonical 64-round expansion;
//! every operation uses the constant-time integer ops native to the CPU.
const H0: [u32; 8] = [
...
#[derive(Clone)]
pub struct Sha256 { h: [u32; 8], len: u64, buf: [u8; 64], n: usize }

impl Sha256 {
    pub fn new() -> Self { Self { h: H0, len: 0, buf: [0; 64], n: 0 } }
    ...
    pub fn update(&mut self, mut data: &[u8]) {
        ...
        if self.n > 0 {
            ...
            self.n += take;
            data = &data[take..];
            if self.n == 64 { self.compress_buf(); self.n = 0; }
        }
        while data.len() >= 64 {
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        ...
        // Pad: 0x80 then zero bytes until length-8 mod 64, then 8-byte BE bit length.
        let mut idx = self.n;
        self.buf[idx] = 0x80; idx += 1;
        if idx > 56 {
            for b in &mut self.buf[idx..64] { *b = 0; }
            self.compress_buf();
            idx = 0;
        }
        for b in &mut self.buf[idx..56] { *b = 0; }
