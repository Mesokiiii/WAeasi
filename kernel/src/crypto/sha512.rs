//! SHA-512 — FIPS 180-4.
//!
//! 64-bit operations, 80 rounds, 128-byte block.  Required by Ed25519
//! (RFC 8032 § 5.1) and HKDF-SHA512 in some TLS profiles.
const H0: [u64; 8] = [
...
#[derive(Clone)]
pub struct Sha512 { h: [u64; 8], len: u128, buf: [u8; 128], n: usize }

impl Sha512 {
    pub fn new() -> Self { Self { h: H0, len: 0, buf: [0; 128], n: 0 } }

    pub fn update(&mut self, mut data: &[u8]) {
        self.len += (data.len() as u128) * 8;
        if self.n > 0 {
            let take = (128 - self.n).min(data.len());
            self.buf[self.n..self.n + take].copy_from_slice(&data[..take]);
            self.n += take;
            data = &data[take..];
            if self.n == 128 { self.compress_buf(); self.n = 0; }
        }
        while data.len() >= 128 {
    ...
    pub fn finalize(mut self) -> [u8; 64] {
        ...
        self.buf[self.n] = 0x80;
        let mut idx = self.n + 1;
        if idx > 112 {
            for b in &mut self.buf[idx..128] { *b = 0; }
            self.compress_buf();
            idx = 0;
        }
        for b in &mut self.buf[idx..112] { *b = 0; }
        self.buf[112..128].copy_from_slice(&total_bits.to_be_bytes());
        self.compress_buf();
